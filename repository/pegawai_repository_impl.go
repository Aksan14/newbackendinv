...
func (r *pegawaiRepositoryImpl) CreatePegawai(ctx context.Context, p model.Pegawai) error {
    query := "INSERT INTO pegawai (nip, email, jabatan, foto) VALUES (?, ?, ?, ?)"
    _, err := r.DB.ExecContext(ctx, query, p.NIP, p.Email, p.Jabatan, p.Foto)
    return err
}

func (r *pegawaiRepositoryImpl) GetAllPegawai(ctx context.Context) ([]model.Pegawai, error) {
    rows, err := r.DB.QueryContext(ctx, "SELECT id, nip, email, jabatan, foto FROM pegawai")
    if err != nil {
        return nil, err
    }
    defer rows.Close()
    ...
    for rows.Next() {
        var p model.Pegawai
        err := rows.Scan(&p.ID, &p.NIP, &p.Email, &p.Jabatan, &p.Foto)
        if err != nil {
            return nil, err
        }
...
func (r *pegawaiRepositoryImpl) GetPegawaiByID(ctx context.Context, id int64) (model.Pegawai, error) {
    ...
    query := "SELECT id, nip, email, jabatan, foto FROM pegawai WHERE id = ?"
    row := r.DB.QueryRowContext(ctx, query, id)

    var p model.Pegawai
    err := row.Scan(&p.ID, &p.NIP, &p.Email, &p.Jabatan, &p.Foto)
    return p, err
}

func (r *pegawaiRepositoryImpl) UpdatePegawai(ctx context.Context, p model.Pegawai) error {
    query := "UPDATE pegawai SET nip=?, email=?, jabatan=?, foto=? WHERE id=?"
    _, err := r.DB.ExecContext(ctx, query, p.NIP, p.Email, p.Jabatan, p.Foto, p.ID)
    return err
}
